...
function getResults(query, page) {
	...
	if (collections.length == 0){
	  ...
	  $("#results").empty();
	  $("#loadingArea").append('<img src="/web/img/loading.gif" />');
	  $("#results").fadeIn(100);
			...
			resultCount = results.getElementsByTagName("totalResults")[0].childNodes[0].nodeValue;
			  ...
			  if (resultCount < 1) {
				...
				for(i=0; i<results.getElementsByTagName("item").length; i++) {
				  ...
				  var collectionText = $(':checkbox[value=' + fromCollection + ']').next("label").text();
				  if (collectionText.indexOf(' (') > -1) {
					  var collectionName = collectionText.substr(0, collectionText.indexOf(' ('));
					  var collectionLink = $(':checkbox[value=' + fromCollection + ']').next("label").find('a').attr('href');
					  var collectionTag = '<h5><span class="glyphicon glyphicon-arrow-right"></span> found in <a href="' + collectionLink + '">' + collectionName + '</a></h5>'
				  } else {
					  var collectionName = collectionText;
					  var collectionTag = '<h5><span class="glyphicon glyphicon-arrow-right"></span> found in ' + collectionName + '</h5>'
				  }
